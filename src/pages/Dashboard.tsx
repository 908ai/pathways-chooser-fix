...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadCompanyInfo = async () => {
      ...
      try {
        ...
        const response = await fetch(`${SUPABASE_URL}/rest/v1/companies?user_id=eq.${user.id}`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      ...
      try {
        ...
        // Account managers can see all projects, regular users only see their own
        const projectsUrl = canViewAllProjects ? `${SUPABASE_URL}/rest/v1/project_summaries?select=*` : `${SUPABASE_URL}/rest/v1/project_summaries?user_id=eq.${user.id}&select=*`;
  ...
  const handleEditProject = (projectId: string, event: React.MouseEvent) => {
    ...
    // Fix #2: Edit Button Fix - Ensure Role Is Set before navigation
    if (roleLoading || !userRole) {
      console.warn('User role not yet loaded, waiting...');
      toast({
        title: "Please Wait",
    ...
    console.log('Editing project with ID:', projectId, 'User role:', userRole);
    console.log('Navigating to:', `/calculator?edit=${projectId}`);
    try {
      ...
      toast({
        ...
        description: "Failed to navigate to project editor",
        variant: "destructive"
      });
    }
  };
  ...
  const handleSaveCompany = async () => {
    ...
    try {
      ...
      if (companyInfo) {
        ...
        response = await fetch(`${SUPABASE_URL}/rest/v1/companies?user_id=eq.${user.id}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Content-Profile': 'api',
        ...
        // Create new company
        response = await fetch(`${SUPABASE_URL}/rest/v1/companies`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Content-Profile': 'api',
      ...
      if (response.ok) {
        ...
        setCompanyInfo(Array.isArray(updatedCompany) ? updatedCompany[0] : updatedCompany);
  ...
  const getPendingItems = (project: any) => {
    ...
    // Check for missing window/door packages
  ...
  const ProjectCard = ({
    ...
    // Helper function to parse project name and separate client name from address
    const parseProjectInfo = (projectName: string) => {
      ...
      // Split by " - " to separate client name from location
    ...
    const {
      clientName,
      location
    } = parseProjectInfo(project.project_name || project.name);
    return <Card className="hover:shadow-md transition-shadow cursor-pointer hover-scale bg-gradient-to-br from-slate-800/60 to-blue-800/60 backdrop-blur-md border-slate-400/30 shadow-2xl" onClick={() => {
      ...
      handleViewProject(project.id);
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            {/* Pending Items Display for In-Progress Projects */}
            ...
            {/* Success indicator for complete projects */}
