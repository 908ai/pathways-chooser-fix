...
const fetchAllProjectsWithCoords = async () => {
  const { data, error } = await supabase
    ...
    .select('id, project_name, location, latitude, longitude, compliance_status')
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          A geographic overview of all projects with location data.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {projects && projects.length > 0 ? (
              ...
              {projects.map(project => (
                <Marker key={project.id} position={[project.latitude, project.longitude]}>
                  <Popup>
                    ...
                    <div className="text-xs mt-1">Status: <span className="font-medium">{project.compliance_status || 'Draft'}</span></div>
          ...
          <Alert>
            ...
            <AlertDescription>
              There are currently no projects with geographic coordinates. For projects to appear on this map, their addresses need to be converted to latitude and longitude.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
