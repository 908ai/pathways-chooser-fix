    ...
    <div className="min-h-screen flex flex-col relative" style={{ backgroundImage: `url(${starryMountainsBg})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundAttachment: 'fixed' }}>
      ...
      <main className="flex-1 container mx-auto px-4 py-8 relative z-10">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-3 text-white drop-shadow-lg">Find a Service Provider</h1>
          <p className="text-gray-200 text-lg drop-shadow-md">
            Browse our directory of trusted professionals.
          </p>
        </div>

        <Card className="mb-8 bg-slate-800/60 backdrop-blur-[100px] border-slate-400/50 shadow-2xl">
          <CardContent className="pt-6 flex flex-col md:flex-row gap-4">
            ...
            <Select value={categoryFilter} onValueChange={setCategoryFilter}>
              <SelectTrigger className="w-full md:w-[200px] bg-slate-900/50 border-slate-600 text-white">
        ...
        {isLoading && <p className="text-center text-white">Loading providers...</p>}
        {error && <p className="text-center text-red-400">Error loading providers: {error.message}</p>}

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProviders?.map(provider => (
            <Card key={provider.id} className="bg-slate-800/60 backdrop-blur-md border-slate-400/30 shadow-lg text-white">
              <CardHeader>
        ...
        {filteredProviders?.length === 0 && !isLoading && (
          <p className="text-center text-slate-300 mt-8">No service providers found matching your criteria.</p>
        )}
      </main>
      <Footer />
    </div>
