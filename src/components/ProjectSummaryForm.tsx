...
const ProjectSummaryForm = ({ calculatorData, onSave, editingProjectId }: ProjectSummaryFormProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Use direct API call with correct schema headers
      ...
      const companyCheckResponse = await fetch(`${SUPABASE_URL}/rest/v1/companies?user_id=eq.${user.id}`, {
        method: 'GET',
        headers: {
      ...
      const existingCompanies = await companyCheckResponse.json();
      ...
      if (shouldUpdateCompany && (calculatorData?.company || calculatorData?.firstName || calculatorData?.lastName)) {
        ...
        if (existingCompanies.length === 0) {
          ...
          if (Object.keys(updateData).length > 0) {
            const companyUpdateResponse = await fetch(`${SUPABASE_URL}/rest/v1/companies?user_id=eq.${user.id}`, {
              method: 'PATCH',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${session.data.session.access_token}`,
      ...
      const projectUrl = isEditing 
        ? `${SUPABASE_URL}/rest/v1/project_summaries?id=eq.${editingProjectId}`
        : `${SUPABASE_URL}/rest/v1/project_summaries`;
      ...
      const response = await fetch(projectUrl, {
        ...
        headers: {
          ...
          'Authorization': `Bearer ${session.data.session.access_token}`,
          'apikey': SUPABASE_KEY,
          'Prefer': isEditing ? 'return=representation' : 'return=minimal'
        },
        body: JSON.stringify({
          ...(isEditing ? {} : { user_id: user.id }), // Only include user_id for new projects
          ...
          window_u_value: parseFloat(formData.windowUValue?.toString() || '0') || 0,
          window_points: formData.windowPoints,
          
          // Mechanical Systems
          heating_system_type: formData.heatingSystemType,
          heating_efficiency: formData.heatingEfficiency?.toString() || '',
      ...
      if (editingProjectId) {
        ...
        // If new project, show success and get the response data for the new project ID
        const responseData = await response.json();
        const newProjectId = responseData?.[0]?.id;
      ...
      // Only call onSave if not editing (so dialog stays open for editing)
      if (!editingProjectId) {
        onSave?.();
      }
    } catch (error) {
